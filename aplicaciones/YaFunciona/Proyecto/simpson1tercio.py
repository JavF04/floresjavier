...
class MetodoSimpson1_3App:
    def __init__(self, on_close_callback):
        self.root = tk.Tk()    
        self.on_close_callback = on_close_callback
        self.root.title("Método de Simpson 1/3")
    ...
    def ingresar_puntos(self):
        try:
            ...
            if num_puntos % 2 != 1 or num_puntos <= 1:
                messagebox.showerror("Error", "El número de puntos debe ser impar y mayor que 1.")
            else:
    ...
    def calcular(self):
        try:
            ...
            obj = MetodoSimpson1_3(valores, distancia)
...
class MetodoSimpson1_3():
    ...
    def calcular_metodo(self):
        ...
        for i in range(1,len(aux_df)-1,2):
            aux_df.loc[i,"c_i"] = round(4, 4)
        for i in range(2, len(aux_df)-2,2):
        ...
        integral_aprox = round((self.diferencia_entre_puntos/3)*sum(aux_df['c_i*f(x)']), 4)
        tabla = str(aux_df)
