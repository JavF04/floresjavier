...
class MetodoTrapecioApp:
    def __init__(self, on_close_callback):
        self.root = tk.Tk()    
        self.on_close_callback = on_close_callback
        self.root.title("Método del Trapecio")
    ...
    def calcular(self):
        try:
            ...
            obj = MetodoTrapecio(valores, distancia)
...
class MetodoTrapecio:
    ...
    def calcular_metodo(self):
        ...
        aux_df["c_i"] = round(self.diferencia_entre_puntos, 4)
        aux_df.loc[0, 'c_i'] = round(self.diferencia_entre_puntos / 2, 4)
        aux_df.loc[len(aux_df) - 1, 'c_i'] = round(self.diferencia_entre_puntos / 2, 4)
        aux_df['c_i*f(x)'] = round(aux_df['c_i'] * aux_df['f(x)'], 4)
        integral_aprox = round(sum(aux_df['c_i*f(x)']), 4)
        tabla = str(aux_df)
